...
% makeODPlots  Unified plotting for ASTE583 EKF + Batch LSQ outputs.
%
% Usage:
%   figs = makeODPlots(out);                         % just plot
%   figs = makeODPlots(out, struct('save',true));    % also save
%
% Expects EKF out to include (typical):
%   out.tk, out.ET0, out.station, out.res_pre_rr, out.res_post_rr,
%   out.res_pre_rho, out.res_post_rho, out.x_hist, out.P_hist, out.x_final, out.P_final
%
% Expects Batch out to include (typical):
%   out.tMeas, out.ET0, out.stationIDs, out.res_prefit(.rr/.rho),
%   out.res_postfit(.rr/.rho), out.X_est, out.P_est, out.iterHist
%

    if nargin < 2, opts = struct(); end
    opts = setDef(opts,'zoomPct',99);        % robust y-limits via percentile of |res|
    ...
    if isEKF
        ...
        pre_rr   = out.res_pre_rr(:);    % km/s
        post_rr  = out.res_post_rr(:);   % km/s
        pre_rho  = out.res_pre_rho(:);   % km
        post_rho = out.res_post_rho(:);  % km
        ...
        x_hist = out.x_hist;     % 10xN
        P_hist = out.P_hist;     % 10x10xN
        x_final = out.x_final(:);
        P_final = out.P_final;
        ...
        pre_rr   = out.res_prefit.rr(:);     % km/s
        post_rr  = out.res_postfit.rr(:);    % km/s
        pre_rho  = out.res_prefit.rho(:);    % km
        post_rho = out.res_postfit.rho(:);   % km
    ...
    % (A) Range-rate residuals by station (prefit/postfit)
    ...
    plotByStation(ax1, t_days, stIDs, pre_rr*1e3, uSt, C);  % m/s
    ...
    % (B) Range residuals by station (only if present)
    ...
    if any(isfinite(pre_rho)) || any(isfinite(post_rho))
        ...
        plotByStation(ax1, t_days, stIDs, pre_rho*1e3, uSt, C); % m
    ...
    % (C) Standardized postfit residuals (time series + histogram)
 
    [sig_rho, sig_rr] = defaultSigmas(kind, stIDs); % km, km/s

    e_rr  = post_rr ./ sig_rr;          % unitless
    e_rho = post_rho ./ sig_rho;        % unitless (NaN where no range)
   ...
   % (D) RMS by station (prefit vs postfit)
    ...
    for i = 1:numel(uSt)
        ...
        rr_pre_rms(i)   = rms_omitnan(pre_rr(ii)*1e3);   % m/s
        rr_post_rms(i)  = rms_omitnan(post_rr(ii)*1e3);  % m/s
    
        rho_pre_rms(i)  = rms_omitnan(pre_rho(ii)*1e3);  % m
        rho_post_rms(i) = rms_omitnan(post_rho(ii)*1e3); % m
    end
    ...
    % Use log y-scale so tiny postfit bars are visible
    set(ax1,'YScale','log');
    ...
    % (E) EKF-only: NIS with chi-square bounds
    ...
    % (F) Batch-only: Convergence history
    ...
    % (G) Final covariance correlation heatmap (both)
    ...
    % (H) 2D Δr_xy + 3σ ellipse (both) — requires X0
    ...
    % Save if requested (only figures made here)
...
function styleResidualAxes(ax, xlab, ylab, zoomPct)
    ...
    % robust symmetric y-lim
    kids = ax.Children;
    yy = [];
    for k = 1:numel(kids)
        if isprop(kids(k),'YData')
            yy = [yy; kids(k).YData(:)]; %#ok<AGROW>
        end
    end
...
function [sig_rho, sig_rr] = defaultSigmas(kind, stIDs)
    % Returns per-measurement sigmas in km and km/s
    sigma_rho_dsn = 1e-3;
    sigma_rho_ant = 1e-2;
    sigma_rr_dsn  = 1e-7;
    sigma_rr_ant  = 1e-6;
    ...
    if kind == "EKF"
        ...
        % Batch defaults (your runBatchLSQ uses scale=5 everywhere)
        Rscale_rr_dsn  = 5;
        Rscale_rr_ant  = 5;
        Rscale_rho_dsn = 5;
        Rscale_rho_ant = 5;
